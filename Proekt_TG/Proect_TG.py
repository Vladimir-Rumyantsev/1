    ...
    def send_text(message):
        try:
            ...
            if ((message.text.lower() == '/start') or (message.text.lower() == 'привет') or
                ...
                bot.send_message(person, f'Привет!')
            else:
                bot.send_message(person, f'Я вас не понял, но привет!')

        except Exception as ex:
            print(ex)
        return person, message.text.lower()
    ...
    while True:
        ...
        for i in range(ln):            # Пройтись по всей БД в поисках дисциплины по требованию пользователя
            if (a.lower() == (str(database[i].name)).lower()) or (a == str(i + 1)): # Если совпадает название или номер
                print(f'\nПодробная информация о дисциплине "{database[i].name}":\n'
    ...
    while True:
        try:                           # Попытаться получить от пользователя число,
            srt = int(input('Выберите номер отчёта, который вы хотите создать:\n'
                            ...
                            '+ читающей кафедре (по возрастанию) + общему количеству часов (по убыванию)\n'
                            'Введите "4" для сортировки дисциплин по другим критериям\n'
                            'Любой другой ввод выведет вас в главное меню\nВаш ввод: '))
            if not (1 <= srt <= 4):    # Если число нас не устраивает - вернуть в главное меню
                print(f'\nВозвращаемся в главное меню\n'
                      f'\n————————————————————————————————————————————————————————————————\n')
                return
        ...
        if srt == 1:                             # Сортировка номер 1
            ...
            srt = str(input('\nВведите "0" или "Выход", чтобы вернуться в главное меню'
